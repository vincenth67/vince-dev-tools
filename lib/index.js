...
async function routeCommand(args) {
    ...
    if (command === 'init') {
        ...
        await init.execute(filteredArgs, { originUrl });
        return;
    }
    if (command === 'merge-main') {
        ...
        await mergeMain.execute(filteredArgs, { force });
        return;
    }
    ...
    try {
        switch (command) {
            case 'init':
                await init.execute(commandArgs);
                break;
            case 'newbranch':
                await newbranch.execute(commandArgs);
                break;
            case 'delbranch':
                await delbranch.execute(commandArgs);
                break;
            case 'commit':
                await commit.execute(commandArgs);
                break;
            case 'version':
                await version.execute(commandArgs);
                break;
            case 'merge-dev':
                await mergeDev.execute(commandArgs);
                break;
            case 'merge-main':
                await mergeMain.execute(commandArgs);
                break;
            case 'status':
                await status.execute(commandArgs);
                break;
            case 'help':
            case '--help':
            case '-h':
