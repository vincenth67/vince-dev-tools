 ...
 * @param {object} [options] - Optional flags (e.g., { force })
 */
async function execute(args, options = {}) {
    ...
    try {
        if (options.force) {
            // Use recursive strategy, favoring develop's version automatically
            executeGitCommand('merge develop --squash --strategy=recursive -X theirs');
            logSuccess('develop squashed into main successfully (force mode, conflicts auto-resolved)');
        } else {
        ...
        if (options.force) {
            // If a conflict still occurs, try to resolve all by favoring develop
            logInfo('Merge conflicts detected. Auto-resolving in force mode...');
            // Try to checkout all conflicted files from develop, add, and continue
            executeGitCommand('checkout --theirs .');
            executeGitCommand('add .');
            // Continue as if merge succeeded
        } else {
