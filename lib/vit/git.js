 ...
 * @returns {boolean}
 */
function isGitUserNameConfigured() {
    try {
        const name = executeGitCommandSilent('config --get user.name');
 ...
 * @returns {boolean}
 */
function isGitUserEmailConfigured() {
    try {
        const email = executeGitCommandSilent('config --get user.email');
...
function setGitUserName(name) {
    executeGitCommand(`config user.name "${name}"`);
}
...
function setGitUserEmail(email) {
    executeGitCommand(`config user.email "${email}"`);
}
